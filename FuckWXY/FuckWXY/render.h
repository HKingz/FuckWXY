...
public:
	...
	void Line(float x, float y, float j, float k, const ColorC &color);

	// rect: w x y h
	void Clear(const Vec4& rect, const ColorC& col);
	void Box(float x, float y, float j, float k, const ColorC& color);
	void Plus(float x, float y, float size, ColorC col);

	void Text(int x, int y, const std::wstring& text, const ColorC& color, uint32_t flags = 1, float fontSize = 12.f);
...
struct Vertex
{
	...
	Vertex(float x, float y, float z, ColorC color) : pos(x, y, z), col(color) {}

	Vec3 pos;
	ColorC col = {0.f, 0.f, 0.f, 0.f};
};
