...
void Render::Line(float x, float y, float j, float k, const ColorC &color)
{
	std::vector<Vertex> v
	{
		{ x, y, 1.0f, color },
...
void Render::Box(float x, float y, float j, float k, const ColorC& color)
{
	Line( x,y, j, y, color);
	Line( j, y, j, k, color);
	Line( j,k, x, k, color);
...
void Render::Clear(const Vec4& rect, const ColorC& color)
{
	std::vector<Vertex> v
	{
...
void Render::Text(int x, int y, const std::wstring& text, const ColorC& color, uint32_t flags, float fontSize)
{
...
void Render::Plus(float x, float y, float size, ColorC col)
{
